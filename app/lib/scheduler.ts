...
//  
//  📌 ثبات الشفت الأسبوعي:
//     - كل موظفة = شفت واحد ثابت طوال الأسبوع
//     - تناوب (1+1): أسبوع 1 صباح → أسبوع 2 مساء → أسبوع 3 صباح → أسبوع 4 مساء
//     - لا يتغير الشفت داخل الأسبوع مهما كانت حالات OFF أو النقص
//  
//  📌 قواعد الإجازات:
//     - الجمعة: OFF للجميع
...
export async function generateSchedule({
  ...
  for (const weekNum of weeks) {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // نظام التناوب (1+1) - أسبوع صباح + أسبوع مساء بالتناوب
    // ═══════════════════════════════════════════════════════════════════
    // الأسبوع 1: المجموعة A صباح، المجموعة B مساء
    // الأسبوع 2: المجموعة A مساء، المجموعة B صباح
    // الأسبوع 3: المجموعة A صباح، المجموعة B مساء
    // الأسبوع 4: المجموعة A مساء، المجموعة B صباح
    // ═══════════════════════════════════════════════════════════════════
    
    const isOddWeek = weekNum % 2 === 1; // أسابيع فردية: 1, 3, 5...
    
    for (const emp of morningGroup) {
      const empId = String(emp.id);
      // المجموعة A: صباح في الأسابيع الفردية، مساء في الزوجية
      shiftMap.set(empId, isOddWeek ? "Morning" : "Evening");
    }
    
    for (const emp of eveningGroup) {
      const empId = String(emp.id);
      // المجموعة B: مساء في الأسابيع الفردية، صباح في الزوجية
      shiftMap.set(empId, isOddWeek ? "Evening" : "Morning");
    }
  }
